...
function showProfileForm(type) {
    ...
    if (type === 'athlete') {
        ...
        document.getElementById('gym-form-title').textContent = 'Gym-Profil anlegen';
...
function editMyProfile() {
    if (myProfile.type === 'athlete') {
        ...
        document.getElementById('athlete-form-title').textContent = 'Profil bearbeiten';
        ...
        document.getElementById('gym-submit-btn').textContent = 'Profil aktualisieren';
        showProfileForm('gym');
    }
}
...
document.getElementById('gym-form').addEventListener('submit', async (e) => {
    ...
    if (isDuplicate) {
        showNotification('Ein Gym mit diesem Namen und dieser Straße existiert bereits!', 'error');
...
async function loadGymsForFilter() {
    ...
    if (gyms && select) {
        select.innerHTML = '<option value="">Alle Gyms</option>' + 
            gyms.map(g => `<option value="${g.id}">${g.name}${g.city ? ` (${g.city})` : ''}</option>`).join('');
    }
}
...
function displayAthletes(athletes) {
    ...
    list.innerHTML = athletes.map(a => {
        const isMyProfile = myProfile && myProfile.type === 'athlete' && myProfile.id === a.id;
...
async function loadOpenMats() {
    ...
    if (data) {
        ...
        list.innerHTML = data.map(om => {
            ...
            const isOwner = myProfile && myProfile.type === 'gym' && om.gyms?.user_id === currentUser.id;
            return `
    ...
    if (createSection) {
        createSection.style.display = (myProfile && myProfile.type === 'gym') ? 'block' : 'none';
    }
}
...
async function loadFriendRequests() {
    ...
    const { data } = await supabase
        .from('friendships')
        .select('*, requester:athletes!friendships_requester_id_fkey(id, name, image_url)')
...
async function loadFriends() {
    ...
    if (data && data.length > 0) {
        list.innerHTML = data.map(f => {
            const friend = f.requester_id === myProfile.id ? f.addressee : f.requester;
            return `
                <div class="profile-card">
                    ${friend.image_url ? `<img src="${friend.image_url}" class="profile-image" alt="${friend.name}">` : 
                      '<div class="profile-image" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center; font-size: 3em; color: white;">👤</div>'}
                    <h3>${friend.name}</h3>
                    ${friend.belt_rank ? `<span class="belt-badge belt-${friend.belt_rank}">${friend.belt_rank.toUpperCase()}</span>` : ''}
                    <button class="btn btn-small" style="margin-top: 10px; width: 100%;" onclick="openChat('${friend.id}')">
                        💬 Chat öffnen
                    </button>
                    <button class="btn btn-small btn-danger" style="margin-top: 5px; width: 100%;" onclick="endFriendship('${f.id}')">
        ...
        list.innerHTML = '<p style="color: #666;">Noch keine Freunde. Sende Freundschaftsanfragen!</p>';
    }
}

async function sendFriendRequest(athleteId) {
    if (!supabase || !myProfile || myProfile.type !== 'athlete') {
        showNotification('Nur Athleten können Freundschaftsanfragen senden', 'warning');
        return;
    }
    ...
    const { data: existing } = await supabase
        .from('friendships')
        .select('id')
        .or(`and(requester_id.eq.${myProfile.id},addressee_id.eq.${athleteId}),and(requester_id.eq.${athleteId},addressee_id.eq.${myProfile.id})`);
...
async function loadChats() {
    ...
    if (friendships && friendships.length > 0) {
        const chatItems = await Promise.all(friendships.map(async f => {
            const friend = f.requester_id === myProfile.id ? f.addressee : f.requester;
        ...
        list.innerHTML = chatItems.map(item => `
            <div class="chat-item ${currentChatPartner === item.friend.id ? 'active' : ''}" onclick="openChat('${item.friend.id}')">
                <div class="name">
                    ${item.friend.name}
                    ${item.unreadCount > 0 ? `<span class="unread-badge">${item.unreadCount}</span>` : ''}
                </div>
                ${item.lastMsg ? `<div class="last-message">${item.lastMsg.message}</div>` : ''}
            </div>
        `).join('');
    } else {
        list.innerHTML = '<p style="color: #666; padding: 10px;">Noch keine Chats</p>';
    }
}
...
async function loadMessages(friendId) {
    ...
    const { data: messages } = await supabase
        ...
        .or(`and(sender_id.eq.${myProfile.id},receiver_id.eq.${friendId}),and(sender_id.eq.${friendId},receiver_id.eq.${myProfile.id})`)
        ...
        messagesDiv.innerHTML = messages.map(m => {
            ...
            return `
                <div class="message ${isOwn ? 'own' : 'other'}">
                    ...
                    <div class="message-time">${date.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
            `;
        }).join('');
...
async function loadOpenMatMessages(openmatId) {
    ...
    if (messagesDiv) {
        messagesDiv.innerHTML = messages.map(m => {
            const isOwn = myProfile && myProfile.type === 'athlete' && m.athlete_id === myProfile.id;
            const date = new Date(m.created_at);
            return `
                <div class="message ${isOwn ? 'own' : 'other'}">
...
document.getElementById('openmat-message-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!supabase || !myProfile || myProfile.type !== 'athlete' || !currentOpenMatChat) return;
